...
from src.api.deps import (
    get_mcsi_clean,
    get_mcsi_uio_summary,
    get_mcsi_vin_status,
    get_sales_targets,
    get_uio_based_demand,
    get_uio_external,
    get_uio_forecast,
...
# Maps each Model value to the base text prefixes in the Material description that
# identify the model variant (not the color).  Multiple entries handle formatting
# inconsistencies within the same model (e.g. space before parenthesis vs none).
_MODEL_BASE_PREFIXES: dict[str, list[str]] = {
    "FZ - V2": ["FZ FI V2"],
    "FZ - V4": ["FZS-FI V4 (STD)", "FZS-FI V4(STD)"],
    "Ray ZR - 125 Disc": ["RAY ZR 125 DISC"],
    "Ray ZR -Disc": ["Ray ZR Disc"],
    "Ray ZR - 125 Street Rally": ["RAY ZR 125 STREET RALLY"],
    "MT 15": ["MT15 V2"],
    "Ray ZR -Street Rally": ["Ray ZR Street Rally"],
    "R-15": ["YZF-R15 V4"],
    "FZ -S- FI": ["FZ-S-FI NEW"],
}


def _extract_color_suffix(model: str, desc: str) -> str | None:
    """Strip the model base name from the Material description; return the color suffix.

    Business meaning: Material descriptions are structured as <model-base> <color-variant>.
    Stripping the base leaves the exact color name as registered in SAP.
    Returns None (→ row excluded from color view) when no color suffix is present.
    """
    for prefix in _MODEL_BASE_PREFIXES.get(model, []):
        if desc.startswith(prefix):
            color = desc[len(prefix) :].strip().strip("()").strip()
...
def get_forecast_by_model() -> ModelForecastResponse:
    ...
    actual_grp = (
        mcsi.groupby(["Year_Month_str", "Model"])
        .size()
...
def get_dealers(
    ...
    if mcsi_full.empty:
        return DealersResponse(
            total_dealers=0, total_units=0, total_revenue_lkr=0.0, available_years=[], rows=[]
        )

    available_years: list[int] = []
    if "Billing Date" in mcsi_full.columns:
        available_years = sorted(
            pd.to_datetime(mcsi_full["Billing Date"], errors="coerce")
            .dt.year.dropna()
            .unique()
            .astype(int)
            .tolist()
    ...
    if mcsi.empty:
        return DealersResponse(
            total_dealers=0,
            total_units=0,
            total_revenue_lkr=0.0,
            available_years=available_years,
            rows=[],
        )

    group_cols = [
        c for c in ["Province", "RM", "ASE", "Dealer", "Dealer Code"] if c in mcsi.columns
    ]
...
@router.get("/dealer-model-matrix", response_model=DealerModelMatrix)
def get_dealer_model_matrix() -> DealerModelMatrix:
    """Dealer × Model units-sold matrix with province / RM / ASE context.

    Business meaning: shows which models each dealer sells, enabling
    stock-mix planning per dealer and identification of under-performing
    model-dealer combinations.
    """
    mcsi = get_mcsi_clean()
    if mcsi.empty or "Model" not in mcsi.columns:
        return DealerModelMatrix(models=[], rows=[])

    dealer_col = next((c for c in ["Dealer", "Dealer Name"] if c in mcsi.columns), None)
    code_col = next((c for c in ["Dealer Code", "Dealer_Code"] if c in mcsi.columns), None)
    if not dealer_col:
        return DealerModelMatrix(models=[], rows=[])

    pivot = mcsi.groupby([dealer_col, "Model"])["VIN"].nunique().unstack(fill_value=0).reset_index()
    models = [c for c in pivot.columns if c != dealer_col]

    # Dealer context: province, RM, ASE, code — first occurrence per dealer
    ctx_cols = [dealer_col] + [
        c for c in [code_col, "Province", "RM", "ASE"] if c and c in mcsi.columns
    ]
    dealer_ctx = mcsi[ctx_cols].drop_duplicates(subset=[dealer_col]).set_index(dealer_col)

    pivot["_total"] = pivot[models].sum(axis=1)
    pivot = pivot.sort_values("_total", ascending=False).drop(columns=["_total"])

    rows: list[DealerModelRow] = []
    for _, r in pivot.iterrows():
        d = str(r[dealer_col])
        ctx = dealer_ctx.loc[d] if d in dealer_ctx.index else pd.Series(dtype=object)
        rows.append(
            DealerModelRow(
                dealer=d,
                dealer_code=str(ctx.get(code_col, "") if code_col else ""),
                province=str(ctx.get("Province", "")),
                rm=str(ctx.get("RM", "")),
                ase=str(ctx.get("ASE", "")),
                total=int(sum(int(r[m]) for m in models)),
                totals={m: int(r[m]) for m in models},
            )
        )

    return DealerModelMatrix(models=models, rows=rows)


@router.get("/geo-model", response_model=GeoModelResponse)
def get_geo_model() -> GeoModelResponse:
    """Geo × Model cross-tab matrices for RM, ASE, Province, and District levels.

    Business meaning: each matrix shows how unit sales distribute across models
    for every entity at that hierarchy level, enabling mix-planning per territory.
    """
    mcsi = get_mcsi_clean()

    def _build_level(col: str | None) -> GeoMatrixLevel:
        if not col or col not in mcsi.columns or "Model" not in mcsi.columns or mcsi.empty:
            return GeoMatrixLevel(models=[], rows=[])
        pivot = mcsi.groupby([col, "Model"])["VIN"].nunique().unstack(fill_value=0).reset_index()
        models = [str(c) for c in pivot.columns if c != col]
        pivot["_total"] = pivot[models].sum(axis=1)
        pivot = pivot.sort_values("_total", ascending=False).drop(columns=["_total"])
        rows = [
            GeoMatrixRow(
        ...
        return GeoMatrixLevel(models=models, rows=rows)

    return GeoModelResponse(
        rm=_build_level("RM" if "RM" in mcsi.columns else None),
        ase=_build_level("ASE" if "ASE" in mcsi.columns else None),
        province=_build_level("Province" if "Province" in mcsi.columns else None),
        district=_build_level("District" if "District" in mcsi.columns else None),
    )


@router.get("/geo-color", response_model=GeoColorResponse)
def get_geo_color() -> GeoColorResponse:
    """Geo × Color cross-tab matrices for RM, ASE, Province, and District levels.

    Business meaning: shows which SAP colour variants sell in each territory,
    enabling region-specific stock-mix decisions (e.g. stock more blue in Northern).
    """
    mcsi = get_mcsi_clean()

    if mcsi.empty or "Model" not in mcsi.columns:
        empty = GeoMatrixLevel(models=[], rows=[])
        return GeoColorResponse(rm=empty, ase=empty, province=empty, district=empty)

    # Attach color column using the same extraction logic as mcsi-eda
    ...
    color_src = mcsi[mcsi["_color"].notna()].copy()

    def _build_color_level(col: str | None) -> GeoMatrixLevel:
        if not col or col not in color_src.columns or color_src.empty:
            return GeoMatrixLevel(models=[], rows=[])
        pivot = (
            color_src.groupby([col, "_color"])["VIN"].nunique().unstack(fill_value=0).reset_index()
        )
        colors = [str(c) for c in pivot.columns if c != col]
        pivot["_total"] = pivot[colors].sum(axis=1)
        pivot = pivot.sort_values("_total", ascending=False).drop(columns=["_total"])
        rows = [
            GeoMatrixRow(
                entity=str(r[col]),
                total=int(sum(int(r[c]) for c in colors)),
                totals={c: int(r[c]) for c in colors},
            )
            for _, r in pivot.iterrows()
        ]
        return GeoMatrixLevel(models=colors, rows=rows)  # "models" field carries color names

    return GeoColorResponse(
        rm=_build_color_level("RM" if "RM" in color_src.columns else None),
        ase=_build_color_level("ASE" if "ASE" in color_src.columns else None),
        province=_build_color_level("Province" if "Province" in color_src.columns else None),
        district=_build_color_level("District" if "District" in color_src.columns else None),
    )
...
def get_mcsi_eda() -> McsiEdaResponse:
    ...
    # By RM / ASE / District — sales hierarchy breakdown
    _rm_col = "RM" if "RM" in mcsi.columns else None
    _ase_col = "ASE" if "ASE" in mcsi.columns else None
    _dist_col = "District" if "District" in mcsi.columns else None

    by_rm: list[McsiRmRow] = []
    if _rm_col and dealer_col:
        rm_grp = (
            mcsi.groupby(_rm_col, dropna=False)
            .agg(
                units_sold=("VIN", "nunique"),
                revenue_lkr=(
                    revenue_col if revenue_col else "VIN",
                    "sum" if revenue_col else "count",
                ),
                dealer_count=(dealer_col, "nunique"),
                ase_count=(_ase_col if _ase_col else _rm_col, "nunique"),
            )
            .reset_index()
            .sort_values("units_sold", ascending=False)
        )
        for _, r in rm_grp.iterrows():
            u = int(r["units_sold"])
            rev = float(r.get("revenue_lkr", 0) or 0)
            by_rm.append(
                McsiRmRow(
                    ...
                    revenue_lkr=round(rev, 0),
                    dealer_count=int(r.get("dealer_count", 0) or 0),
                    ase_count=int(r.get("ase_count", 0) or 0),
                    share_pct=round(u / total_sold * 100, 1) if total_sold else 0.0,
                    avg_revenue_per_unit=round(rev / u, 0) if u else 0.0,
                )
            )
    ...
    if _ase_col and dealer_col:
        grp_cols: list[str] = [_ase_col] + ([_rm_col] if _rm_col else [])
        ase_grp = (
            mcsi.groupby(grp_cols, dropna=False)
            .agg(
                units_sold=("VIN", "nunique"),
                revenue_lkr=(
                    revenue_col if revenue_col else "VIN",
        ...
        for _, r in ase_grp.iterrows():
            u = int(r["units_sold"])
            rev = float(r.get("revenue_lkr", 0) or 0)
            by_ase.append(
                McsiAseRow(
                    ase=str(r[_ase_col]),
                    rm=str(r[_rm_col]) if _rm_col else "",
                    units_sold=u,
                    revenue_lkr=round(rev, 0),
                    dealer_count=int(r.get("dealer_count", 0) or 0),
                    share_pct=round(u / total_sold * 100, 1) if total_sold else 0.0,
    ...
    if "Province" in mcsi.columns and _dist_col:
        dist_grp = (
            mcsi.groupby(["Province", _dist_col], dropna=False)
            .agg(
                units_sold=("VIN", "nunique"),
                revenue_lkr=(
                    revenue_col if revenue_col else "VIN",
        ...
        for _, r in dist_grp.iterrows():
            u = int(r["units_sold"])
            by_district.append(
                McsiDistrictRow(
                    province=str(r["Province"]),
                    district=str(r[_dist_col]),
                    units_sold=u,
                    revenue_lkr=round(float(r.get("revenue_lkr", 0) or 0), 0),
                    share_pct=round(u / total_sold * 100, 1) if total_sold else 0.0,
                )
    ...
    # By color — strip model base prefix from Material description to get exact SAP color name
    desc_series = mcsi["Material"].str.split(r"\s{2,}", n=1, expand=True)
    mcsi = mcsi.copy()
    mcsi["_desc"] = desc_series[1].str.strip() if 1 in desc_series.columns else ""
    mcsi["_color"] = mcsi.apply(
        lambda row: _extract_color_suffix(str(row.get("Model", "")), str(row["_desc"])),
        axis=1,
    )
    ...
    if not color_src.empty and "Model" in color_src.columns:
        color_grp = (
            color_src.groupby(["Model", "_color"])
            .agg(units_sold=("VIN", "nunique"))
            .reset_index()
            .rename(columns={"_color": "color"})
            .sort_values(["Model", "units_sold"], ascending=[True, False])
        )
        by_color = [
            McsiColorRow(
                model=str(r["Model"]), color=str(r["color"]), units_sold=int(r["units_sold"])
            )
            for _, r in color_grp.iterrows()
        ]
...
@router.post("/targets")
def write_targets(body: dict = Body(...)) -> dict:  # noqa: B008
...
def get_uio_demand(
    ...
    supply_pct = (
        float(df["supply_pct_applied"].iloc[0]) if "supply_pct_applied" in df.columns else 0.0
    )
